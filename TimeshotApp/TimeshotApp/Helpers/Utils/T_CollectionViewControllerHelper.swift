//
//  T_CollectionViewControllerHelper.swift
//  TimeshotApp
//
 ...
 */
public protocol CollectionTimelineComponentTarget: class {
 ...
 */
public class CollectionTimelineComponent <T: Equatable, S: CollectionTimelineComponentTarget where S.ContentType == T> : Refreshable {
    ...
    public init(target: S) {
        ...
        target.collectionView.insertSubview(refreshControl, atIndex:0)
...
//    }
    ...
    public func loadInitialIfRequired() {
        ...
        if (content == []) {
            target!.loadInRange(target!.defaultRange) { posts in
                ...
                self.target!.collectionView.reloadData()
            }
        }
    }
    ...
    public func refresh(sender: AnyObject) {
        ...
        target!.loadInRange(target!.defaultRange) { content in
            ...
            UIView.transitionWithView(self.target!.collectionView,
                                      duration: 0.35,
                                      options: .TransitionCrossDissolve,
                                      animations:
                { () -> Void in
                    self.target!.collectionView.reloadData()
                    self.target!.collectionView.contentOffset = CGPoint(x: 0, y: 0)
                },
                                      completion: nil);
        }
    }
